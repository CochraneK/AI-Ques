...
function peerProfile(){
  if(!CORE.canImportP005(consentState()))return {};
  const raw=read(K.p005,null),profile=raw&&raw.profile&&typeof raw.profile==='object'?raw.profile:{};
  const allowed=['name','age','gender','location','currentWork','values'];
  return Object.fromEntries(allowed.filter(k=>profile[k]).map(k=>[k,profile[k]]));
}
...
function renderPersona(){const p=peerProfile();$('personaName').textContent=p.name||'你的 Persona';$('personaAvatar').textContent=(p.name||'我').slice(0,1)}
...
function setP005Consent(enabled){
  ...
  renderConsent();renderPersona();renderSourceCounts();
}

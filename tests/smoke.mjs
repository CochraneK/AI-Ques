...
const expectedModes = ["original", "vassip", "rush"];

assert.deepEqual(
  Array.from(evaluate("Object.keys(MODES)")).sort(),
  [...expectedModes].sort(),
  "P002 runtime registry should expose exactly the three active measurement forms"
);
...
console.log("P002 smoke tests passed: 2 scales × 3 active modes, registry + manifest + start/end paths.");
...
assert.ok(!appSource.includes("Emoji Game 式"), "Emoji should not return as a primary measurement mode");
